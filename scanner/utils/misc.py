# -*- coding: utf-8 -*-
"""
Created on 30 Oct 2020 22:10:16
@author: jiahuei
...
assert os.path.isdir(
    os.path.join(REPO_DIR, "scanner")
), f"`REPO_DIR` should point to `scanner`, saw `{REPO_DIR}` instead."
...
def round_to_nearest_odd(x):
    ...
    if xi % 2 == 1:
        # +/- 1.x, 3.x, 5.x, etc
        return xi
    elif xu % 2 == 1 and abs(xu - x) <= 1:
        # 2.x, 4.x, 6.x, etc
        return xu
    elif xd % 2 == 1 and abs(xd - x) <= 1:
        # -2.x, -4.x, -6.x, etc
        return xd
    else:
        # Should not get here
...
def numpy_tolist(array, decimals):
    ...
    def _round(arr):
        if isinstance(arr, (float, int)):
            ...
            return [_round(_) for _ in arr]

    if not isinstance(array, np.ndarray):
        raise TypeError(
            f"Expected `array` of type `np.ndarray`, saw {type(array)}"
        )
    array = array.tolist()
    return _round(array)
